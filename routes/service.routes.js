...
router.post('/', isAuthenticated, isAdmin, async (req, res) => {
  try {
    ...
    const populatedService = await Service.findById(newService._id).populate('comments.user', 'username _id profilePicture role');
...
router.put('/:id', isAuthenticated, isAdmin, async (req, res) => {
  try {
    const updatedService = await Service.findByIdAndUpdate(req.params.id, req.body, { new: true }).populate('comments.user', 'username _id profilePicture role');
...
router.get('/', async (req, res) => {
    try {
        ...
        const allServices = await Service.find(query)
            .populate('comments.user', 'username _id profilePicture role')
            .populate('reviews.user', 'username profilePicture')
            .sort(sortQuery);
            
        // ✅ SÉCURITÉ : On nettoie les commentaires et avis des services pour retirer 
        // ceux dont l'utilisateur a été supprimé.
        const cleanServices = allServices.map(service => {
            const serviceObj = service.toObject();
            if (Array.isArray(serviceObj.comments)) {
                serviceObj.comments = serviceObj.comments.filter(comment => comment.user);
            }
            if (Array.isArray(serviceObj.reviews)) {
                serviceObj.reviews = serviceObj.reviews.filter(review => review.user);
            }
            return serviceObj;
        });

        res.status(200).json(cleanServices);
    } catch (error) {
...
router.patch('/:id/like', isAuthenticated, async (req, res) => {
  try {
    ...
    await service.save();
    const updatedService = await Service.findById(id).populate('comments.user', 'username _id profilePicture role');
    req.io.emit('serviceUpdated', updatedService);
    res.status(200).json(updatedService);
  } catch (error) {
    res.status(500).json({ message: 'Erreur interne du serveur.' });
...
router.put('/:serviceId/comments/:commentId', isAuthenticated, async (req, res) => {
  try {
    ...
    comment.text = text;
    await service.save();
    const updatedService = await Service.findById(serviceId).populate('comments.user', 'username _id profilePicture role');
    req.io.emit('serviceUpdated', updatedService);
    res.status(200).json(updatedService);
