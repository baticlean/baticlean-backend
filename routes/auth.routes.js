...
router.post('/login', async (req, res) => {
  try {
    ...
    // Si l'utilisateur est banni ou suspendu, on envoie quand même un token
    // pour que le client puisse écouter les événements de réactivation.
    if (user.status !== 'active') {
      return res.status(403).json({
        message: 'Votre compte a été suspendu ou banni.',
        // On fournit le token pour garder l'écouteur actif côté client
        authToken: authToken,
        // On envoie aussi les informations utilisateur non sensibles
        user: payload
      });
    }

    // Cas d'une connexion réussie pour un utilisateur actif
